...
Use when coaching a learner through Jimmy Song's *Programming Bitcoin*, especially early math chapters.
...
Learners find dense inline equations hard to follow. Use the vertical step format instead.
...
When the learner asks "how does division work in a finite field?" do NOT start with Fermat's formula. Start with the brute-force intuition:
...
If the learner says "that doesn't click," stay on step 1-2 longer with tiny fields (F_7, F_13) before introducing Fermat.

## Structured equation walkthroughs

When the learner needs to see *how* numbers go into an equation, use this structured format — it worked well during Chapter 3's curve check:
...
- Keep explanations synced to the learner's current page. If they say they have not reached modulo/Fermat/finite-field shortcuts yet, stop using that as the main explanation and give only a hook.
...
- For exercises, first explain what the exercise is asking and the shape of the work, not the final answer. Give one worked example only if needed, then let the learner do the rest.
- For Chapter 2 point-addition geometry, use diagrams early. Learners benefit from generated/drawn visuals showing: line through A and B → third intersection → reflect; vertical-line inverse → point at infinity; associativity as two paths to the same final point. Pair the image with a short explanation of what is chosen first and what is forced by the graph.
...
- For visual-heavy explanations in chat, prefer broken-down diagram/example images with the explanation immediately after each image. Learners learn better when each example is paired with the exact explanation that references it.
...
- In Obsidian notes, place each image immediately above the section it explains, copy assets into a local `assets/` folder, and run `ob sync` from `/home/clawdbot/vault` after updates so the learner's devices receive the changes.
...
- The book **switches curves between chapters.** Chapter 2 uses `y² = x³ + 5x + 7` (a=5, b=7) as a teaching curve. Chapter 3 switches to `y² = x³ + 7` (a=0, b=7 — the real secp256k1 curve). Do not carry Chapter 2's `a=5` into Chapter 3 examples or notes. The learner will get wrong answers and be confused because none of the points pass the check.
...
- When explaining "over F_p", emphasize that it means **both sides of the equation reduce by % p before comparing.** Not one side. Not optionally. The "over F_p" wraps the entire equality: `y² % p == (x³ + ax + b) % p`. This is where learners get stuck most.
- Do not answer finite-field exponent questions with only the compact formula. The learner may need value provenance.
...
- Do not use full exercise solutions unless the learner asks for rescue or has already attempted and is blocked.
- When labeling P1 and P2 in worked examples, always use the book's formula order (`s = (y2 - y1) / (x2 - x1)`) and label x1/y1/x2/y2 clearly. Mixing the order confuses the learner — they will catch swapped labels.
